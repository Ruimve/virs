...
## 决策辅助计算（开仓前必须完成）
请基于上述15分钟指标计算以下数值，并在分析中引用：
- 布林带半宽 = ({m15_bb_upper} - {m15_bb_lower}) / 2
- 入场预期收益% = 布林带半宽 / {m15_current_price} × 100
- ATR占比 = {m15_atr} / 布林带半宽
- ATR占比判断：< 0.5 优质入场 | 0.5-0.7 标准入场 | 0.7-1.0 边际入场(仅路径C) | > 1.0 放弃入场
- 布林带宽度判断：{m15_bb_width_pct}% > 0.4% 为可交易状态
