...
impl SymbolCache {
    ...
    pub(crate) fn update_candle(&mut self, timeframe: Timeframe, candle: Candle) {
        if let Some(buf) = self.timeframes.get_mut(&timeframe) {
            buf.push_or_update(candle);
        }
    }

    pub(crate) fn close_candle(&mut self, timeframe: Timeframe, open_time: i64) {
        if let Some(buf) = self.timeframes.get_mut(&timeframe) {
            buf.close_candle(open_time);
        }
    }

    pub(crate) fn get_klines(&self, timeframe: Timeframe) -> Vec<Candle> {
        self.timeframes
            .get(&timeframe)
            .map(|buf| buf.get_all())
            .unwrap_or_default()
    ...
    pub(crate) fn last_closed_1m(&self) -> Option<Candle> {
        self.timeframes
            .get(&Timeframe::M1)
    ...
    pub(crate) fn replace_timeframe(&mut self, timeframe: Timeframe, candles: Vec<Candle>) {
        if let Some(buf) = self.timeframes.get_mut(&timeframe) {
