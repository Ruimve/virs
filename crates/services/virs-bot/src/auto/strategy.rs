...
pub fn render_prompt(template: &str, ctx: &PromptContext) -> String {
    ...
    template
        ...
        .replace(
            "{margin_usage_rate}",
            &format!("{:.1}", ctx.margin_usage_rate * 100.0),
        )
        ...
        .replace(
            "{h4_macd_histogram}",
            &format!("{:.4}", ctx.ind.h4_macd_histogram),
        )
        .replace("{h4_adx}", &format!("{:.2}", ctx.ind.h4_adx))
        .replace(
            "{h1_current_price}",
            &format!("{:.2}", ctx.ind.current_price),
        )
        ...
        .replace(
            "{h1_ema_gap_pct}",
            &format!("{:.2}", ctx.ind.h1_ema_gap_pct),
        )
        ...
        .replace(
            "{m15_macd_signal}",
            &format!("{:.4}", ctx.ind.m15_macd_signal),
        )
        .replace(
            "{m15_macd_histogram}",
            &format!("{:.4}", ctx.ind.m15_macd_histogram),
        )
    ...
    match current_side {
        Some(side) if !side.is_empty() && side != "none" => {
            ...
            let liq_str = position
                ...
                .map(|p| format!("\n- 强平价格：{:.2}", p))
                .unwrap_or_else(|| {
                    tracing::warn!("Liquidation price unavailable — omitting from display");
                    String::new()
                });
            format!(
                "- 方向：{}\n- 入场价：{:.2}\n- 持仓量：{:.6}\n- 当前价：{:.2}\n- 未实现盈亏：{:.4} USDT ({:+.2}%){}",
