...
/* PE订单执行器：将BotEngine的OrderCommand转换为PE的EngineCommand，
 * 并通过virs-task启动事件转发任务将PE的EngineEvent转换为OrderEvent广播给BotEngine */
...
/* 引擎事件到订单事件的映射：将PE的EngineEvent（持仓/订单状态变更）转换为BotEngine可消费的OrderEvent */
