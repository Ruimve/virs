...
struct EngineState {
    paper_mode: bool,
    bot_cmd_tx: StdMutex<Option<mpsc::Sender<BotCommand>>>,
...
impl EngineManager for AppEngineManager {
    ...
    async fn ensure_started(&self, paper_mode: bool) -> VirsResult<()> {
        ...
        let bot_store = Arc::new(PgBotStore::new(self.db_pool.clone()));
        ...
        /* App层装配BotEngine：将具体adapter实现强制转换为trait object传入，
         * 包括Arc<dyn KlineEventSource>、Arc<dyn PromptProvider>、Option<Arc<dyn StrategyHotSwapSource>> */
        let (bot_cmd_tx, auto_task) = virs_trading_bot::create_bot_engine(
            bot_store,
            auto_llm_resolver,
        ...
        let _ = self.state.set(EngineState {
            paper_mode,
            bot_cmd_tx: StdMutex::new(Some(bot_cmd_tx)),
    ...
    fn bot_cmd_tx(&self) -> Option<mpsc::Sender<BotCommand>> {
        self.state
            .get()
            .and_then(|s| s.bot_cmd_tx.lock().unwrap().clone())
    }
    ...
    async fn shutdown(&self) {
        if let Some(state) = self.state.get() {
            ...
            drop(state.bot_cmd_tx.lock().unwrap().take());
