...
pub struct PerpetualHandler {
    ws: Arc<Mutex<dyn KlineWsClient>>,
}

impl PerpetualHandler {
...
mod tests {
    ...
    #[async_trait]
    impl KlineWsClient for MockPerpetualWs {
    ...
    fn make_handler() -> PerpetualHandler {
        PerpetualHandler::new(Arc::new(Mutex::new(MockPerpetualWs::new())))
    }

    #[tokio::test]
    async fn test_perpetual_handler_market_type() {
        let handler = make_handler();
        assert_eq!(handler.market_type(), MarketType::Perpetual);
    }

    #[tokio::test]
    async fn test_perpetual_handler_start_stop() {
        let handler = make_handler();
    ...
    #[tokio::test]
    async fn test_perpetual_handler_subscribe() {
        let handler = make_handler();
    ...
    #[tokio::test]
    async fn test_perpetual_handler_unsubscribe() {
        let handler = make_handler();
        handler.subscribe("BTCUSDT").await;
    ...
    #[tokio::test]
    async fn test_perpetual_handler_not_running_initially() {
        let handler = make_handler();
        assert!(!handler.is_running().await);
    }

    #[tokio::test]
    async fn test_perpetual_handler_idempotent_start() {
        let handler = make_handler();
        let (tx, _) = broadcast::channel(16);
        handler.start(tx).await;
        assert!(handler.is_running().await);
