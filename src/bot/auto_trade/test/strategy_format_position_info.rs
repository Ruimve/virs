...
fn long_position_shows_side() {
    let info = format_position_info(Some("long"), 100.0, 0.1, 110.0, None);
...
fn long_position_unrealized_pnl_positive() {
    let info = format_position_info(Some("long"), 100.0, 1.0, 110.0, None);
    assert!(info.contains("+10.00%"), "should show +10% pnl, got: {}", info);
}

#[test]
fn long_position_unrealized_pnl_negative() {
    let info = format_position_info(Some("long"), 100.0, 1.0, 90.0, None);
    assert!(info.contains("-10.00%"), "should show -10% pnl, got: {}", info);
}

#[test]
fn short_position_unrealized_pnl_positive() {
    let info = format_position_info(Some("short"), 100.0, 1.0, 90.0, None);
    assert!(info.contains("+10.00%"), "should show +10% pnl for short, got: {}", info);
}

#[test]
fn short_position_unrealized_pnl_negative() {
    let info = format_position_info(Some("short"), 100.0, 1.0, 110.0, None);
    assert!(info.contains("-10.00%"), "should show -10% pnl for short, got: {}", info);
}

#[test]
fn no_position_none_string() {
    assert_eq!(format_position_info(Some("none"), 0.0, 0.0, 0.0, None), "无仓位");
}

#[test]
fn no_position_empty_string() {
    assert_eq!(format_position_info(Some(""), 0.0, 0.0, 0.0, None), "无仓位");
}

#[test]
fn no_position_none_option() {
    assert_eq!(format_position_info(None, 0.0, 0.0, 0.0, None), "无仓位");
}

#[test]
fn zero_entry_price_pnl_pct_is_zero_with_sign() {
    let info = format_position_info(Some("long"), 0.0, 1.0, 110.0, None);
    assert!(info.contains("+0.00%"), "pnl pct should be +0.00% for zero entry, got: {}", info);
}

#[test]
fn liquidation_price_shown_when_present() {
    let info = format_position_info(Some("long"), 100.0, 1.0, 110.0, Some(85.0));
    assert!(info.contains("强平价格"), "should contain liquidation price label, got: {}", info);
    assert!(info.contains("85.00"), "should contain liquidation price value, got: {}", info);
}

#[test]
fn liquidation_price_hidden_when_absent() {
    let info = format_position_info(Some("long"), 100.0, 1.0, 110.0, None);
    assert!(!info.contains("强平价格"), "should not contain liquidation price when None, got: {}", info);
}
