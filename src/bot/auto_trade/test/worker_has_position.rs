/**
 * 测试 AutoWorker::has_position 仓位状态判断
 * - current_side = Some("long") → true
 * - current_side = Some("short") → true
 * - current_side = Some("none") → false
 * - current_side = Some("") → false
 * - current_side = None → false
 */
...
fn make_worker(mut config: AutoBotConfig) -> AutoWorker {
