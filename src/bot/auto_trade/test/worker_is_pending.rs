/**
 * 测试 AutoWorker::is_pending / is_spot / is_long / is_short / normalize_side
 * - is_pending: pending_open 或 pending_close 存在时为 true
 * - is_spot: MarketType::Spot 时为 true
 * - is_long/is_short: current_side 匹配时为 true
 * - normalize_side: 有效方向返回 Some, 无效返回 None
 */
...
#[test]
fn is_pending_no_pending() {
    let worker = make_worker(make_bot_config());
    assert!(!worker.is_pending());
}

#[test]
fn is_spot_perpetual() {
    let worker = make_worker(make_bot_config());
...
#[test]
fn is_spot_spot() {
    let mut config = make_bot_config();
...
fn is_long_true() {
    let mut config = make_bot_config();
    config.current_side = Some("long".to_string());
    let worker = make_worker(config);
    assert!(worker.is_long());
    assert!(!worker.is_short());
}

#[test]
fn is_short_true() {
    let mut config = make_bot_config();
    config.current_side = Some("short".to_string());
    let worker = make_worker(config);
    assert!(!worker.is_long());
    assert!(worker.is_short());
}
...
#[test]
fn normalize_side_long() {
    let mut config = make_bot_config();
    config.current_side = Some("long".to_string());
    let worker = make_worker(config);
    assert_eq!(worker.normalize_side(), Some("long"));
}

#[test]
fn normalize_side_short() {
    let mut config = make_bot_config();
    config.current_side = Some("short".to_string());
    let worker = make_worker(config);
    assert_eq!(worker.normalize_side(), Some("short"));
}

#[test]
fn normalize_side_none_string() {
    let mut config = make_bot_config();
    config.current_side = Some("none".to_string());
    let worker = make_worker(config);
    assert_eq!(worker.normalize_side(), None);
}

#[test]
fn normalize_side_none_option() {
    let worker = make_worker(make_bot_config());
    assert_eq!(worker.normalize_side(), None);
}

#[test]
fn normalize_side_empty() {
    let mut config = make_bot_config();
    config.current_side = Some("".to_string());
    let worker = make_worker(config);
    assert_eq!(worker.normalize_side(), None);
}
