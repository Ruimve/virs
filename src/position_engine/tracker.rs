...
mod tests {
    use super::*;
    use crate::position_engine::types::{Position, PositionSide, PositionStatus, Side};
    use chrono::Utc;
    use uuid::Uuid;
    ...
    fn make_trade(pnl: f64, trade_type: &str) -> Trade {
        Trade {
            id: Uuid::new_v4(),
    ...
    fn test_snapshot_win_rate() {
        ...
        tracker.record_trade(&make_trade(100.0, "close"));
        tracker.record_trade(&make_trade(50.0, "close"));
        tracker.record_trade(&make_trade(-30.0, "close"));
        tracker.record_trade(&make_trade(200.0, "close"));

        let snap = tracker.snapshot(0.0, 0);
        assert_eq!(snap.win_rate, Some(0.75));
    }

    #[test]
    fn test_update_unrealized_long() {
        let mut tracker = PnlTracker::new(10000.0);
        ...
        // Long: (55000 - 50000) * 1.0 = 5000
        assert!((snap.unrealized_pnl - 5000.0).abs() < 0.01);
        assert!((snap.equity - 15000.0).abs() < 0.01);
    }

    #[test]
    fn test_update_unrealized_short() {
        let mut tracker = PnlTracker::new(10000.0);

        let pos = make_position("BTCUSDT", PositionSide::Short, 1.0, 50000.0, 10);
        let mut prices = HashMap::new();
        prices.insert("BTCUSDT".to_string(), 45000.0);

        let snap = tracker.update_unrealized(&[&pos], &prices);

        // Short: (50000 - 45000) * 1.0 = 5000
        assert!((snap.unrealized_pnl - 5000.0).abs() < 0.01);
    }
    ...
    fn test_pnl_ratio() {
        ...
        tracker.record_trade(&make_trade(300.0, "close"));
        tracker.record_trade(&make_trade(-100.0, "close"));

        let snap = tracker.snapshot(0.0, 0);
        // pnl_ratio = 300 / 100 = 3.0
        assert_eq!(snap.pnl_ratio, Some(3.0));
    }

    #[test]
    fn test_open_cost_tracking() {
        let mut tracker = PnlTracker::new(10000.0);

        tracker.record_trade(&make_trade(0.0, "open"));
        // cost = 50000 * 0.1 + 1.0 = 5001.0
        assert!((tracker.total_cost() - 5001.0).abs() < 0.01);

        // close trade should not add cost
        tracker.record_trade(&make_trade(100.0, "close"));
        assert!((tracker.total_cost() - 5001.0).abs() < 0.01);
    }
}
