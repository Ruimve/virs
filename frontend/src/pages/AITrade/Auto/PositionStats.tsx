...
const PositionStats = ({ bot }: PositionStatsProps) => {
  const b = bot
  ...
  return (
    <div className="px-3 py-2 flex items-center gap-3 overflow-x-auto text-xs scrollbar-none">
      ...
      {hasPosition ? (
        <>
          <span
            className={`font-medium ${b.current_side === 'long' ? 'text-emerald-400' : 'text-red-400'}`}
          >
          ...
          <span className="shrink-0">{formatPnl(b.unrealized_pnl)}</span>
        </>
      ) : (
      ...
      {/* Stats */}
      <span className="shrink-0">累计 {formatPnl(b.total_pnl)}</span>
