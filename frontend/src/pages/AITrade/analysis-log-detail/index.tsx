    ...
    <div className="min-h-screen bg-base">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-6">
        <div className="space-y-4">
          ...
          <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                {decision && (
                  <span className={`text-xs font-medium px-2 py-0.5 rounded ${getActionColor(decision.action)}`}>
                    {getActionLabel(decision.action)}
                  </span>
                )}
                {log.status === 'failed' && (
                  <span className="text-xs font-medium px-2 py-0.5 rounded bg-red-500/10 text-red-400">失败</span>
                )}
              </div>
            ...
            {decision?.confidence != null && (
              <div className="text-[11px] text-on-surface-tertiary">
                置信度: <span className="text-on-surface-secondary">{(decision.confidence * 100).toFixed(0)}%</span>
              </div>
            )}
          </div>
          ...
          {decision && (
            <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
              <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-2">决策</div>
              {decision.reason && (
                <p className="text-xs text-on-surface-secondary leading-relaxed">{decision.reason}</p>
              )}
            </div>
          )}
          ...
          {log.result?.analysis && (
            <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
              <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-2">AI 分析</div>
              <p className="text-xs text-on-surface-secondary leading-relaxed whitespace-pre-wrap">{log.result.analysis}</p>
            </div>
          )}
          ...
          {log.result?.market && (
            <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
              <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-2">市场状态</div>
              <div className="space-y-1">
                ...
                {log.result.market.funding_rate_warning && (
                  <p className="text-xs text-amber-400">⚠ 资金费率: {log.result.market.funding_rate_warning}</p>
                )}
                {log.result.market.event_impact && (
                  <p className="text-xs text-purple-400">事件影响: {log.result.market.event_impact}</p>
                )}
              </div>
            </div>
          )}
          ...
          {log.result?.grid && (log.result.grid.upper_price || log.result.grid.lower_price) && (
            <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
              <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-2">网格参数</div>
              <div className="flex flex-wrap gap-3 text-xs text-on-surface-secondary">
                ...
                {log.result.grid.grid_profit_pct && <span>利润率 {log.result.grid.grid_profit_pct}%</span>}
              </div>
            </div>
          )}
          ...
          {log.result?.risk && (log.result.risk.leverage || log.result.risk.quantity_per_grid) && (
            <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
              <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-2">风控参数</div>
              <div className="flex flex-wrap gap-3 text-xs text-on-surface-secondary">
                {log.result.risk.leverage && <span>杠杆 {log.result.risk.leverage}x</span>}
                {log.result.risk.quantity_per_grid && <span>每格 {log.result.risk.quantity_per_grid} USDT</span>}
              </div>
            </div>
          )}
          ...
          {log.result?.risk_warning && log.result.risk_warning !== 'none' && (
            <div className="bg-amber-500/5 rounded-xl border border-amber-500/20 p-5">
              <div className="text-[10px] text-amber-400 uppercase tracking-wider mb-2">风险提示</div>
          ...
          {log.system_prompt && (
            <div className="bg-surface-1 rounded-xl border border-line-default shadow-sm overflow-hidden">
              ...
              >
                <svg className={`w-3 h-3 transition-transform ${showSystemPrompt ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              ...
              {showSystemPrompt && (
                <div className="px-5 pb-4">
                  <pre className="text-[11px] text-on-surface-secondary bg-surface-2 rounded-lg p-3 overflow-x-auto whitespace-pre-wrap break-all max-h-64 overflow-y-auto font-mono leading-relaxed border border-line-subtle">{log.system_prompt}</pre>
                </div>
              )}
            </div>
          )}
          ...
          {log.user_prompt && (
            <div className="bg-surface-1 rounded-xl border border-line-default shadow-sm overflow-hidden">
              <button
                ...
                className="w-full flex items-center gap-2 px-5 py-3 text-[10px] text-on-surface-tertiary uppercase tracking-wider hover:text-on-surface-secondary transition-colors"
              >
                <svg className={`w-3 h-3 transition-transform ${showUserPrompt ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                </svg>
                User Prompt
              </button>
              {showUserPrompt && (
                <div className="px-5 pb-4">
                  <pre className="text-[11px] text-on-surface-secondary bg-surface-2 rounded-lg p-3 overflow-x-auto whitespace-pre-wrap break-all max-h-64 overflow-y-auto font-mono leading-relaxed border border-line-subtle">{log.user_prompt}</pre>
                </div>
              )}
            </div>
          )}
          ...
          {log.result?.raw_llm_response && (
            <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
              <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-2">LLM 原始响应</div>
          ...
          {log.result && !log.result.raw_llm_response && (
            <div className="bg-surface-1 rounded-xl border border-line-default p-5 shadow-sm">
              <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-2">完整结果</div>
              <pre className="text-[11px] text-on-surface-secondary bg-surface-2 rounded-lg p-3 overflow-x-auto whitespace-pre-wrap break-all max-h-64 overflow-y-auto font-mono leading-relaxed border border-line-subtle">{JSON.stringify(log.result, null, 2)}</pre>
            </div>
          )}
        </div>
      </div>
