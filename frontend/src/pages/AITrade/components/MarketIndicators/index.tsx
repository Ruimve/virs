...
export default function MarketIndicators({ klineData, orderBook }: MarketIndicatorsProps) {
  ...
  return (
    <div className="px-4 py-2 border-b border-line-subtle">
      <div className="text-[10px] text-on-surface-tertiary uppercase tracking-wider mb-1.5">市场指标</div>
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className={`font-mono ${bidPressure > 55 ? 'text-emerald-400' : bidPressure < 45 ? 'text-red-400' : 'text-on-surface'}`}>
        ...
        <div className="flex items-center justify-between col-span-2">
          ...
          <span className="text-on-surface font-mono">{low.toFixed(2)} — {high.toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
