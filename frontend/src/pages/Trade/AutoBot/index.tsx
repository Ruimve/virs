    ...
    const items: NavItem[] = [
      {
        ...
        label: '机器人',
        icon: navIcons.bot,
        onClick: () => navigate(`/trade/auto/${bot?.id}/bot`, { replace: true }),
      },
      {
        key: 'trades',
        label: '交易记录',
        icon: navIcons.trades,
        onClick: () => navigate(`/trade/auto/${bot?.id}/trades`, { replace: true }),
      },
      {
        key: 'log',
        label: 'AI 决策日志',
        icon: navIcons.log,
        onClick: () => navigate(`/trade/auto/${bot?.id}/log`, { replace: true }),
      },
      {
        key: 'system',
        label: '系统',
        icon: navIcons.system,
        onClick: () => navigate(`/trade/auto/${bot?.id}/system`, { replace: true }),
      },
    ];
    setNavItems(items);
